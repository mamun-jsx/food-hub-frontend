...
export default function DashboardSidebar() {
  ...
  return (
    <nav className="bg-background border-r h-screen w-64 fixed md:relative">
      <div className="px-6 py-6">
        <h1 className="text-2xl font-bold text-primary mb-8">FoodHub</h1>
        ...
        <ul className="space-y-2">
          {navItems.map((item) => (
            <li key={item.name}>
              <Link
                href={item.href}
                className="block px-4 py-2 rounded hover:bg-primary/10 text-gray-800 font-medium"
              >
                {item.name}
              </Link>
            </li>
